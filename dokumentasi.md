...
- **Dihapus dari alur (dibanding model lama):** 3 preset tetap, mode persentase Custom, `PresetPicker`, mode `terpisah`/`kombinasi` + shortfall, toggle `Include_Savings` (tabungan saat ini kini selalu saldo awal), dan cabang "Arah A time-to-goal" (horizon kini selalu diberikan). Persentase tidak pernah menjadi input.
- **Logika murni** di `lib/planner/goalBudget.ts` (`computeGoalBudget`) — akumulasi murni, diuji PBT (`fast-check`). Modul lama `presets.ts`/`budget.ts`/`savingsProjection.ts` **dipensiunkan tetapi dipertahankan** di repo (tak dipakai UI/API goal-driven).
